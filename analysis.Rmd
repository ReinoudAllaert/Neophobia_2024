...
# 13/30 not eating trials are from birds that did not eat twice or more 
...
ggplot(data_not_eat, aes(x = obj_cont, y = Count, fill = Context)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = Count), 
            position = position_dodge(width = 0.9), 
            vjust = -0.3, size = 3) +
  labs(title = "Object x context for birds that never ate",
       x = "Object x context",
       y = "Count") +
  theme_minimal()

# Almost all of those occurences are individual x novel
...
```{r, fig.width=10, fig.height=10, dpi=300}
latency_model3_boxcox_reduced <- lmer(Latency_trans ~ eat_vs_leave_contrast * Object_contrast +
...
```
latency_model3_boxcox_reduced seems to be the best fit, but gives warnings (contrary to reduced_2):
...
```
###For ZOI, now object is no longer significant (but interaction is):
So when birds are in group, they don't care about the novelty of the object, when alone they do.
